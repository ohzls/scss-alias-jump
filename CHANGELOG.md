...
### Fixed
- Added a hard timeout race for in-flight workspace scans so a cancellation-stalled `workspace.findFiles` call cannot keep later identical scans waiting forever.

### Added
- Added `scssAliasJump.cacheAutoClearIntervalMs` for periodic internal cache self-healing.
- Added `SCSS Alias Jump: Clear Internal Caches` command for manual recovery without reloading Cursor.

### Improved
- Automatic cache reset now clears Sass path lookup, usage caches, queued scans, and in-flight scan state on a schedule and on relevant style-file saves.
